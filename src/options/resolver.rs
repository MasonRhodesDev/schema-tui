...
impl OptionResolver {
    ...
    pub async fn resolve(&mut self, source: &OptionSource, values: &HashMap<String, Value>) -> Result<Vec<String>> {
        match source {
            OptionSource::Static { values } => Ok(values.clone()),
            
            OptionSource::Script { command, cache_duration, .. } => {
                self.resolve_from_script(command, *cache_duration, values).await
            }
    ...
    async fn resolve_from_script(&mut self, command: &str, cache_duration: Option<u64>, values: &HashMap<String, Value>) -> Result<Vec<String>> {
    ...
    pub fn resolve_from_script_sync(&self, command: &str, values: &HashMap<String, Value>) -> Result<Vec<String>> {
    ...
    fn substitute_variables(command: &str, values: &HashMap<String, Value>) -> Result<String> {
        ...
        for cap in re.captures_iter(command) {
            ...
            let replacement = values.get(var_name)
                .map(|v| match v {
...
mod tests {
    ...
    fn test_substitute_variables() {
        ...
        values.insert("daemon.language".to_string(), Value::String("en".to_string()));
